...
class Window(QtWidgets.QWidget):
    ...
    def build_tree(self):
        for index in range(10):
            ...
            push_button.clicked.connect(self.the_button_was_clicked)
            slider = QtWidgets.QSlider(QtCore.Qt.Horizontal)

            self.treewidget.setItemWidget(item, 2, push_button)
            self.treewidget.setItemWidget(item, 3, slider)
            

    def the_button_was_clicked(self):
        num = self.treewidget.topLevelItemCount()
        for i in range(num):
            item = self.treewidget.topLevelItem(i)
            slide = self.treewidget.itemWidget(item, 3)
            print(f"i:{i} val:{slide.value()}")
